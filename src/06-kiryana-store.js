...
export function sortByPrice(items, ascending) {
  // Your code here
  if(!Array.isArray(items) || items.length === 0) return [];
  const newAscendingArr = [...items].sort((a,b) => a.price - b.price);
  const newDescendingArr = [...items].sort((a,b) => b.price - a.price);
  if(ascending) return newAscendingArr;
  return newDescendingArr;
}

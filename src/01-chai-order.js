...
export function hasSpecialIngredient(order, ingredient) {
  ...
  let myOrder = (order.trim()).toLowerCase();
  let myIngredient = (ingredient.trim()).toLowerCase();
  if(myOrder.includes(myIngredient)) return true;
  return false;
}
...
export function getFirstAndLastChar(order) {
  ...
  let myOrder = order.trim();
  return {first: myOrder.charAt(0), last: myOrder.at(-1)}
}
